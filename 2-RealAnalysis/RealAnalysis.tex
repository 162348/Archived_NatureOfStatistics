...
\begin{definition}[extended real number]
...
\begin{proposition}[\cite{Stein-Shakarchi-05-RealAnalysis} Th'm 1.3,1.4 開集合の区間塊としての理解]\mbox{}
    \begin{enumerate}
...
\begin{lemma}[区間塊は$\delta$-環である \cite{Tao11-MeasureTheory} Exercise 1.1.1]
    $E,F\in\F_d$を区間塊とする．
    \begin{enumerate}
...
\begin{theorem}[Jordan可測性の特徴付け \cite{Tao11-MeasureTheory} Exercise 1.1.5]
...
\begin{corollary}[一般化されたLusinの定理 (Rudin 2.24\cite{Rudin})]
...
\begin{theorem}[Vitali-Caratheodory]
...
\begin{definition}[concrete Boolean algebra \cite{Tao10-AnEpsilonOfRoom} 2.3]
...
\begin{proposition}[\cite{Tao10-AnEpsilonOfRoom} 2.3]
...
\begin{definition}[rectable / cylinder set, 直積$\sigma$-加法族]
...
\begin{definition}[Vitali covering]\mbox{}
    \begin{enumerate}
...
\begin{definition}[symmetric derivative, Hardy–Littlewood maximal function]\mbox{}
    \begin{enumerate}
...
\begin{definition}[shrink nicely]
...
\begin{theorem*}[Lebesgue (再掲\ref{thm-Lebesgue-fundamental})]
...
\begin{definition}[anti-derivative, indefinite integral]\mbox{}
    \begin{enumerate}
...
\begin{theorem}[Lebesgue (1904)：不定積分が原始関数になる条件]
...
\begin{theorem}[微積分学の基本定理：原始関数は不定積分である]
...
\begin{example}[原始関数$G$が可積分な導関数をもつとは限らない]
    $G:[-1,1]\to\R$を
    \[G(x)=\begin{cases}
...
\begin{theorem}[Lebesgue (1902)：至る所可微分な連続関数が有界変動であるための必要十分条件は不定積分であること]\label{thm-Lebesgue-BV-and-L1}
...
\begin{definition}[purely finitely additive]
...
\begin{theorem}[Hausdorff-Youngの不等式]
    $p\in[1,2]$とする．このとき，
    \begin{enumerate}
...
\begin{theorem}[Sobolev空間の$L^p$-部分空間としての特徴付け]
