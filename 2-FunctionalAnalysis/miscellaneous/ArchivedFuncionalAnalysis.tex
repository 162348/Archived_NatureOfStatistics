...
\begin{lemma}[Riesz空間 / 束線型空間，関数束]\mbox{}
    \begin{enumerate}
        ...
        \item $C_c(X)^m$は正錐であり，また，任意の２つの正な元の積について閉じている：$C_c(X)^m_+\cdot C_c(X)^m_+\subset C_c(X)^m$．
...
title=]
    ...
    特に第２可算だと任意のBorel確率測度がRadonかつ正則になる．
\end{tcolorbox}
...
\begin{lemma}[Borel測度がRadonになるとき]\mbox{}
    \begin{enumerate}
        ...
        \item 他に，第２可算な局所コンパクトハウスドルフ空間上のBorel確率測度は正則になる．
...
\begin{corollary}[Jordan decomposition]
    任意の実数値Radon電荷$\Phi:C_c(X)\to\R$について，２つのRadon積分$\int_+,\int_-$が存在して，互いに素なBorel集合内に台を持ち，$\Phi=\int_+-\int_-$と表せる．
\end{corollary}
...
\begin{proposition}
    $X$が局所コンパクトかつ第１可算ならば，$\o{M_+(X)}$の漠位相は距離化可能である．
\end{proposition}
...
\begin{definition}
    $(u_\al)_{\al\in A}$を正規直交系とする．
    \begin{enumerate}
        \item 次で定まる線型写像$\wh{-}:H\to\Map(A;\C)$をFourier変換，値を\textbf{Fourier係数}という．
        \[\wh{x}(\al):=(x|u_\al)\quad(\al\in A).\]
        \item $\varphi:A\to\o{\R_+}$について，$\sum_{\al\in A}\varphi(\al):=\sup_{F\subset A,\abs{F}<\infty}\sum_{\al\in F}\sum_{\al\in A}\varphi(\al)$．
    \end{enumerate}
\end{definition}
\begin{remarks}
    これは，$A$上の数え上げ測度を$\mu$として，$\sum_{\al\in A}\varphi(\al):=\int_A\varphi d\mu$と定めたことに他ならない．
    上述の定義でうまくいく理由は，$\varphi=0\;\fe$を満たす関数が$l^2(A)$上稠密であることによる．
    実際，$\varphi\in l^2(A)$は$\Brace{\al\in A\mid\varphi(\al)\ne0}$は高々可算である．
\end{remarks}

\begin{theorem}[Fourier部分和]
    $\{u_\al\}_{\al\in A}\subset H$を正規直交系，$F\subset A$を有限部分集合，$M_F:=\Span\{u_\al\}_{\al\in F}$を有限次元部分空間とする．
    \begin{enumerate}
        \item $\varphi:A\to\C$が$A\setminus F$上零ならば，ある$y\in M_F$が存在して
        \begin{enumerate}[(a)]
            \item $\forall_{\al\in A}\;\wh{y}(\al)=\varphi(\al)$を満たし，
            \item $\norm{y}^2=\sum_{\al\in F}\abs{\varphi(a)}^2$を満たす．
        \end{enumerate}
        \item $x\in H$と$s_F(x):=\sum_{\al\in F}\wh{x}(\al)u_\al$について，
        \begin{enumerate}[(a)]
            \item $\forall_{s\in M_F}\;s\ne s_F(x)\Rightarrow\norm{x-s_F(x)}<\norm{x-s}$で，
            \item $\sum_{\al\in F}\abs{\wh{x}(\al)}^2\le\norm{x}^2$．
        \end{enumerate}
        すなわち，Fourier級数$\sum\wh{x}(\al)u_\al$は，有限次元部分空間$M_F$内で$x$に最も近い点である．
    \end{enumerate}
\end{theorem}
\begin{Proof}\mbox{}
    \begin{enumerate}
...
\subsection{Hilbの標準的同型}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=対応するFourier変換が同型を引き起こすものが正規直交基底]
    任意のHilbert空間に対して，正規直交系からFourier変換$H\epi l^2(A)$が定まり，これは有界線型作用素であるが，さらにHilbの同型であることが，正規直交系$\{u_\al\}$が基底でもあることを特徴づける(Parsevalの等式)．
\end{tcolorbox}

\begin{lemma}[Metの同型の十分条件]
    $X$を完備距離空間，$Y$を距離空間とする．
    連続関数$f:X\to Y$が，ある稠密部分集合$X_0$上で等長で，像$f(X_0)$が$Y$上稠密ならば，$f$は全射な等長写像である．
\end{lemma}

\begin{theorem}[Riesz-Fischer]
    $\{u_\al\}_{\al\in A}\subset H$を正規直交系，$P\subset H$を$\{u_\al\}$の有限線形結合が生成する部分空間とする．このとき，
    \begin{enumerate}
        \item (Besselの不等式) Fourier変換$\wh{-}:H\to l^2(A)$が定まり，ノルム減少的で全射な連続線形写像になる：$\sum_{\al\in A}\abs{\wh{x}(\al)}^2\le\norm{x}^2$．
        \item 制限$\wh{-}|_{\o{P}}:\o{P}\to l^2(A)$は全射な等長写像，すなわちHilbの等長同型を定める．
    \end{enumerate}
\end{theorem}
\begin{Proof}\mbox{}
    \begin{enumerate}
...
\end{Proof}
\begin{remarks}
    この証明は殆ど$L^2$の完備性によるので，$L^2$が完備であるという結果自体もRiesz-Fischerの定理と呼ばれることがある．
\end{remarks}

\begin{theorem}[正規直交系の完全性の特徴付け]
    $\{u_\al\}_{\al\in A}\subset H$を正規直交系とする．次の4条件は同値：
    \begin{enumerate}
        \item $\{u_\al\}\subset H$は正規直交系の中でも極大である．
        \item $P$は$H$上稠密である．
        \item $\forall_{x\in H}\;\sum_{\al\in A}\abs{\wh{x}(\al)}^2=\norm{x}^2$．
        \item (Parseval) $\forall_{x,y\in H}\;(\wh{x}|\wh{y})_{l^2(A)}=\sum_{\al\in A}\wh{x}(\al)\o{\wh{y}(\al)}=(x|y)_H$．
    \end{enumerate}
    すなわち，$\{u_\al\}$が定めるFourier変換$\wh{-}:H\epi l^2(A)$が可逆なとき，これを基底という．
    $H=\bF^n$のとき，$l^2(n)=\bF^n$であったのだ！
\end{theorem}
\begin{Proof}\mbox{}
    \begin{description}
        \item[(1)$\Rightarrow$(2)] 
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item $A\simeq_\Set B$．
        \item $l^2(A)\simeq_\Hilb l^2(B)$．
    \end{enumerate}
\end{lemma}

\begin{theorem}
    Hilbert空間について，次の2条件は同値：
    \begin{enumerate}
        \item $H$は可分である．
        \item $H$は可算な正規直交基底を持つ．
    \end{enumerate}
\end{theorem}

\subsection{Fourier級数の収束}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^2(T)$-収束について，Fourier変換論が定まる．
    しかし，各点収束の議論は難しく，68年にやっと$1<p\le\infty$については概収束について決着した．
\end{tcolorbox}

\begin{corollary}
    任意の$f\in L^2(T)$に対して，Fourier部分和
    \[s_N(t)=\sum^N_{n=-N}\wh{f}(n)e^{int},\quad\wh{f}(n):=(f*e_{n})(0)\]
    は$f$に$L^2(T)$-収束する：$\lim_{N\to\infty}\norm{f-s_N}_2=0$．
\end{corollary}
\begin{Proof}
    Parsevalの定理(Fourier級数への対応$L^2(T)\to l^2(\Z)$がHilbert空間の同型を定めること)により，
    \[\norm{f-s_N}_2^2=\sum_{\abs{n}>N}\abs{\wh{f}(n)}^2.\]
\end{Proof}
\begin{remarks}
    したがって，$f\in L^2(T)$のFourier級数は概収束する部分列も持つことがわかった．
\end{remarks}

\begin{theorem}[Lipschitz連続関数の各点収束]
    $f$が$x$においてLipschitz連続ならば，$\lim_{N\to\infty}s_N(f;x)=f(x)$が成り立つ．
\end{theorem}

\begin{tbox}{red}{}
    ここから各点収束や一様収束を議論することは難しくなる．また，$L^2(T)\mono L^1(T)$への延長も難しくなる．
    \begin{theorem}[Carleson(1966), Hunt (1968)]\mbox{}
        \begin{enumerate}
            \item $f\in L^p(T)\;(1<p\le\infty)$について，Fourier級数は$f$に殆ど至る所収束する：$\lim_{N\to\infty}\sum_{\abs{n}\le N}\wh{f}(n)e^{inx}=f(x)\;\ae$
            \item $f\in L^p(\R)\;(1<p\le 2)$について，Fourier積分は$f$に殆ど至る所収束する：$\lim_{R\to\infty}\int_{\abs{\xi}\le R}\wh{f}(\xi)e^{2\pi ix\xi}d\xi=f(x)\;\ae$
        \end{enumerate}
    \end{theorem}
\end{tbox}

\subsection{Fourier級数の発散点}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $C(T)\mono L^\infty(T)\mono L^2(T)\mono L^1(T)$に話を限って，各点収束のミニチュア論を考える．
\end{tcolorbox}

\begin{notation}[畳み込みとしてのFourier変換を見ればBanach空間論に落ちる]
    Fourier部分和は，Dirichlet核との畳み込み
    \[s_n(f;x)=\frac{1}{2\pi}\int^\pi_{0\pi}f(t)D_n(x-t)dt=(f*D_n)(x)\]
    で与えられるから，$D_n$がどういう意味で近似的単位元になっているかを考えることに同値．
    実際，$D_n$は近似的単位元であるが，正な元からなるわけではないので，十分良い振る舞いをしない．
    特に，$L^1$-ノルムは収束しない．すなわち，一様可積分ではない．
    したがって$L^1$-収束しない．
    しかしFejer核を取れば任意のLebesgue点の上で収束する(特に$L^1$-収束する，はず)．
\end{notation}

\begin{corollary}[Banach-Steinhausの系]
    稠密な$G_\delta$集合$E\subset C(T)$が存在して，任意の$f\in E$に対して
    \[Q_f:=\Brace{x\in\R\mid s^*(f;x):=\sup_{n\in\N}\abs{s_n(f;x)}=\infty}\]
    は$\R$上の稠密な$G_\delta$集合である．
\end{corollary}
\begin{lemma}
    完備距離空間$X$が孤立点を持たないとき，可算稠密集合は決して$G_\delta$でない．
    またこのとき，$X$の任意の稠密な$G_\delta$重合は完全集合を含む．
\end{lemma}

\subsection{Fourier級数近似の一般化}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=Riesz-Fischerの定理がBanach空間から延長されるとBanの埋め込みに堕ちる]
    次は逆方向に，$L^2(T)\mono L^1(T)$にFourier変換$\F:L^2(T)\to l^2(\Z)$を延長出来ないか考える．
    $L^1(T)$はもはやHilbert空間ではないが，$L^2(T)\simeq_\Hilb l^2(\Z)$が得られたように，
    $L^1(T)\simeq_\Ban C_0(\Z)$に延長することは出来ないだろうか？
    実は，埋め込みになってしまい，全射にはならない．
\end{tcolorbox}

\begin{lemma}[Riemann-Lebesgue]
    任意の$f\in L^1(T)$について，$\wh{f}\in C_0(\Z)$．
\end{lemma}
\begin{Proof}
    $C(T)$は$L^1(T)$上稠密だから，三角多項式の全体$P$は$L^1(T)$上稠密でもある．
    Riesz-Fischerの定理よりFourier変換は$L^2$-ノルム閉包$\o{P}$上に，特に$P$上に等長写像$\wh{-}:P\to l^2(\Z)$を定める．
    よって$f\in L^1(T)$の像は特に$C_0(\Z)$から出ない．
\end{Proof}

\begin{theorem}[Fourier変換の延長は埋め込み]
    Fourier変換$\wh{-}:L^1(T)\to c_0$は単射な有界線型作用素であるが，全射ではない．
\end{theorem}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
    title=Fourier変換論の前の級数論]
    Fourier変換のコンパクト群$T$に対する理論の特殊化として，周期関数$f\in L^1(\R^n)$には，係数への反変対応$\F:L^1(T)\mono C_0(\Z)=:c_0$として捉えられる．
    これは抽象化するとBanach代数$L^1(T)$上の近似的単位元を探す理論である．
    \begin{enumerate}
        \item Dirichlet核$\{D_n\}\subset L^1(T)$は近似的単位元であるが，ノルムが発散するので一様有界でない．
        \item これを少し修正したFejer核$\{K_n\}\subset L^1(T)_+$は(畳み込みに対して)正な元からなる近似的単位元で，有界集合をなす．
        \item Poisson核$\{P_r\}\subset L^1(T)$は$0\le r<1$について定まる$r\to1$に関する近似的単位元で，これに関する畳み込みは調和関数を与える．
    \end{enumerate}
    (2),(3)は総和可能核である．
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
$T^n$上の任意の分布はコンパクト台を持つ．特に，全ての超関数は緩増加である．
\end{tcolorbox}

\begin{proposition}
    $\wt{-}:\Map(T^n;\C)\to\Map(\R^n;\C)_{2\pi}$は，$\wt{\phi}(x_1,\cdots,x_n)=\phi(e^{ix_1},\cdots,e^{ix_n})$とすることで集合として同一視出来る．
\end{proposition}

\begin{notation}
    これを用いて，$\D(T^n)=\Brace{\phi\in\Map(T^n;\C)\mid\wt{\phi}\in C^\infty(\R^n)}$となる．
    $D(T^n)$には，各$N$が定めるノルム
    \[\paren{\sum_{k\in\Z^n}(1+k\cdot k)^N\abs{\wh{\phi}(k)}^2}^{1/2}<\infty\]
    が定まり，これが誘導する位相についてFrechet空間になる．
    これは$\max_{\abs{\al}\le N}\sup_{x\in\R^n}\abs{(D^\al\wt{\phi})(x)}$が与える位相と同じ．
\end{notation}

\begin{definition}
    $\sigma_n$を$T^n$のHaar測度，$\phi\in L^1(\sigma_n)$とする．
    \begin{enumerate}
        \item $k\in\Z^n$に対して，$e_k:T^n\to\C$を$e_k(e^{ix_1},\cdots,e^{ix_n})=e^{ik\cdot x}$で定める．
        \item $\phi$のFourier係数とは，次によって定まる関数$\wh{\phi}:\Z^n\to\C$をいう：
        \[\wh{\phi}(k):=\int_{T^n}e_{-k}\phi d\sigma_n\]
    \end{enumerate}
\end{definition}

\begin{proposition}[超関数の特徴付け]
    $u:\Z^n\to\C$について，
    \begin{enumerate}
        \item $\exists_{N\in\N}\;\exists_{C\in\R}\;\forall_{k\in\Z^n}\;\abs{\wh{u}(k)}\le C(1+\abs{k})^N$．
        \item $u\in\D'(T^n)$
    \end{enumerate}
    すなわち，$T^n$上の分布と，$Z^n$上の多項式増加関数とに全単射が存在する．
\end{proposition}

\subsection{畳み込み}

\begin{definition}
    $u,v\in\D'(T^n)$について，Fourier係数$\wh{u}(k)\wh{v}(k)$が定めるものとして$u*v$を定める．
\end{definition}

\subsection{抽象基底}

\begin{proposition}
    $f_n(t):=e^{int}\in L^p(-\pi,\pi)\;(1\le p<\infty)$とする．このとき，$f_n\wto 0$が成り立つが，ノルム収束はしない．
\end{proposition}

\begin{theorem}
    Hilbert空間内の任意の正規直交列は$0$に弱収束する．
\end{theorem}

\begin{proposition}
    $f\in L^1([0,2\pi])$のFourier係数
    \[\wt{f}(n)=(2\pi)^{-1}\int^{2\pi}_0f(x)\exp(-inx)dx\]
    は無限遠$n\to\pm\infty$で消える．
\end{proposition}

\subsection{三角多項式をなす基底}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^2(T)$に標準的に取れる基底$e^{int}$に関する線型和を三角多項式という．
    三角多項式は$L^2(T)$上稠密であるどころか，Fourierの対応によって$f$に対して係数をうまく定めれば，三角多項式の極限として$f$を復元出来る(Fejerの定理)．
\end{tcolorbox}
...
\begin{lemma}[trigonometric system]
    ...
    $(u_n)_{n\in\Z}$は
    \begin{enumerate}
        \item $L^2(T)$の正規直交系である：
        \[(u_n,u_m)=\frac{1}{2\pi}\int^\pi_{-\pi}e^{i(n-m)t}dt=\begin{cases}
            1,&n=m,\\
            0,&n\ne m.
        \end{cases}\]
        \item 任意の$f\in C(T)$と$\ep>0$について，ある三角多項式$P$が存在して$\forall_{t\in T}\;\abs{f(t)-P(t)}<\ep$を満たす．
        特に，$(u_n)_{n\in\Z}$が生成する部分空間は$L^2(T)$上稠密であるため，基底でもある．
    \end{enumerate}
    これを三角多項式系という．
\end{lemma}
...
\begin{definition}\mbox{}
    \begin{enumerate}
        \item $D_k(x):=\sum^k_{s=-k}e^{isx}$を\textbf{Dirichlet核}という．
        \item Dirichlet核の算術平均$K_n(x):=\frac{1}{n}\sum^{n-1}_{k=0}D_k(x)$を\textbf{Fejer核}という．
    \end{enumerate}
\end{definition}
...
\begin{lemma}
    Dirichlet核について，
    \begin{enumerate}
        \item $D_n(x)=1+2\sum^n_{k=1}\cos(kx)=\frac{\sin((n+1/2)x)}{\sin(x/2)}$．
    \end{enumerate}
    Fejer核について，
    \begin{enumerate}
        \item $F_n(x)=\frac{1}{n}\paren{\frac{\sin\frac{nx}{2}}{\sin\frac{x}{2}}}^2$．
        \item $F_n\ge0$．
        \item $F_n$に関する畳み込み$-*F_n:L^1(T)\to L^1(T)$は正な作用素である．
        \item $(F_n)$は近似的単位元をなす：$\forall_{f\in L^1(T)}\;f*F_n\to f$．$f$が連続なら一様収束する．
        \item $L_N(t)=\min(N,\pi^2/Nt^2)$とすると，$K_N(t)=\frac{1}{N}\frac{1-\cos Nt}{1-\cos t}\le L_N(t)$が成り立ち，$\int_TL_Nd\sigma\le 2$が成り立つ．
    \end{enumerate}
\end{lemma}

\begin{theorem}[Fejer (1904)]
    $f\in C(T)$の定めるFourier級数の部分和
    $P_n(x)=\sum^n_{k=-n}c_ke^{ikx}$
    の算術平均
    \[\sigma_n(x):=\frac{1}{n}\sum^{n-1}_{k=0}s_k(x)=\frac{1}{2\pi}\int^\pi_{-\pi}f(x-t)F_n(t)dt\]
    は$T$上$f$に一様収束する．
\end{theorem}

\begin{theorem}[Dini]
    $f\in L^1(T)$についても，$f$の任意のLebesgue点の上で$f$に収束する．
\end{theorem}

\subsection{Taylor展開がなす基底}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $e_n$の完全性とTaylor展開を組み合わせると，多項式の完全性も得る．
    その中で，特殊な多項式系も完全になる．
\end{tcolorbox}

\begin{proposition}
    $e_n(z):=(2\pi)^{-1/2}z^n\;(n\in\Z)$はHilbert空間$L^2(T)$の正規直交基底をなす．
\end{proposition}

\begin{corollary}[Weierstrassの定理]
    $I\subset\R$をコンパクトな区間とする．
    \begin{enumerate}
        \item $C(I)$において，多項式は一様ノルムについて稠密である．
        \item $L^p(I)\;(1\le p<\infty)$においても，多項式の全体は稠密である．
    \end{enumerate}
\end{corollary}

\begin{proposition}\mbox{}
    \begin{enumerate}
        \item $L^2([-1,1];dx)$において，$\{1,x,x^2,x^3,\cdots\}$を正規直交化して得られる多項式系をLegendre多項式という．これは完全である．
        \item $L_n(x):=\frac{e^x}{n!}\dd{^n}{x^n}(x^ne^x)\;(n\in\N)$をLagerreの多項式系という．これは$L^2(\R_+,e^{-x}dx)$における完全直交系である．
        \item $H_n(x):=(-1)^ne^{x^2}\dd{^n}{x^n}e^{-x^2}\;(n\in\N)$をHermiteの多項式系という．これは$L^2(\R,e^{-x^2}dx)$における完全直交系である．
    \end{enumerate}
\end{proposition}

\begin{proposition}[Hilbert変換]
    $W\in B(L^2(T))$を
    \[We_0=e_0,\quad We_n=-ie_n,\quad We_{-n}=ie_{-n}\quad(n\in\N)\]
    によって定めると，
    \begin{enumerate}
        \item これはユニタリで，$L^2(T)$の実関数を実関数に写す．
        \item $\forall_{x\in L^2(T)}\;x+iWx\in H^2$．
        \item $x\in L^2(T)$を$\Re f$の境界条件として持つ2乗可積分正則関数$f\in A^2$を求めるDirichlet問題が，虚数分の不訂正を除いて一意に解ける．
    \end{enumerate}
\end{proposition}

\subsection{Toeplitz作用素}

\begin{theorem}
    $H^2\subset L^2(T)$が定める射影を$P:L^2(T)\epi H^2$とする．任意の$f\in L^\infty(T)$について，\textbf{Toeplitz作用素}$T_f\in B(H^2)$を$T_fx=P(fx)\;(x\in H^2)$によって定める．
    \begin{enumerate}
        \item 対応$T:L^\infty(T)\to B(H^2);f\mapsto T_f$はノルム減少的な有界線型作用素である．
        \item $T_f^*=T_{\o{f}}$．
        \item $\forall_{n\in\N}\;T_fe_n=\sum_{m=0}^\infty\wh{f}(m-n)e_m$．
        \item $T_f$が定める行列は対角成分が一定である．
        \item 特に$f=e_k\;(k\in\Z)$のとき，対応する作用素$T_f$はユニラテラルシフト作用素の$k$乗またはその随伴である．
        \item $T_f$がコンパクト作用素ならば，$f=0$．
        \item $f,g\in L^\infty(T)$が連続ならば，$T_fT_g-T_gT_f$と$T_fT_g-T_{fg}$とはいずれもコンパクト作用素である．
        \item 特に$f\in C(T)$で$f$が可逆なとき，$T_f$はFredholm作用素であり，$u:=f\abs{f}^{-1}$について$\Index T_f=\Index T_u$．また，$f$の$0$の周りの回転数は$-\Index T_f$である．
        \item 一般に，$f,g\in C(T,T)$がホモトピックならば，$\Index T_f=\Index T_g$を満たし，Fredholm型が一致する．
        \item $T|_{C(T)}:C(T)\to B(H^2)$は等長写像であり，$r(T_f)=\norm{T_f}$を満たす．
    \end{enumerate}
\end{theorem}

\subsection{Poisson核がなす基底}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^1(T)$の元はPoisson核の無限和で表せる．
\end{tcolorbox}

\begin{theorem}
    $X$をBanach空間，
    $\{x_n\}\subset B$を列とし，$\exists_{\delta>0}\;\forall_{x^*\in X^*}\;\sup_{n\in\N}\abs{\brac{x_n,x^*}}\ge\delta\norm{x^*}$とする．
    このとき，任意の$\ep>0$と$x\in X$に対して，$\delta\sum_{n=1}^\infty\abs{c_n}\le\norm{x}+\ep$を満たす係数列$\{c_n\}$が存在して$x=\sum^\infty_{n=1}c_nx_n$を満たす．
\end{theorem}

\begin{example}
    $L^1:=L^1(T:=\partial\Delta)$のノルムを
    \[\norm{f}_1=\frac{1}{2\pi}\int^\pi_{-\pi}\abs{f(e^{i\theta})}d\theta\]
    とし，$P:\Delta\to L^1(T)$を
    \[P_z(e^{i\theta})\frac{1-\abs{z}^2}{\abs{e^{i\theta}-z}^2}\quad(z\in\Delta)\]
    で定めると，$\Im P\subset T$を満たす．
\end{example}

\begin{definition}
    $E\subset\Delta$が$T$上\textbf{非接触稠密}であるとは，$\forall_{e^{i\theta}\in T}\;\forall_{\ep>0}\;\exists_{z\in E}\;\abs{z-e^{i\theta}}<\min(\ep,2(1-\abs{z}))$を満たすことをいう．
\end{definition}

\begin{theorem}[Bonsall, F. F.]
    $\{z_n\}\subset\Delta$は$T$上非接触稠密であるとする．このとき，任意の$f\in L^1(T)$と$\ep>0$に対して$\{c_n\}\subset\R$が存在して，
    \[\sum_{n\in\N}\abs{c_n}\le\norm{f}_1+\ep\qquad f=\sum_{n\in\N}c_nP_{z_n}\]
    を満たす．
\end{theorem}

\subsection{Poisson核の特徴付け}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $\Delta$上の点を極表示し，$0\le r<1$と$\theta\in T$に分ける．
\end{tcolorbox}

\begin{proposition}
    $P:\Delta\to L^1(T)$をPoisson核とする．
    \begin{enumerate}
        \item $r\in(0,1)$について，$P_r(\theta)=\sum^\infty_{-\infty}r^{\abs{n}}e^{in\theta}$とも表せる．
        \item $P_r(\theta)=\frac{1-r^2}{1-2r\cos\theta+r^2}=\Re\paren{\frac{1+re^{i\theta}}{1-re^{i\theta}}}$．
    \end{enumerate}
\end{proposition}

\begin{proposition}
    $f\in L^1(T)$について，$u:\Delta\to\C$を畳み込み
    \[u(re^{i\theta}):=(f*P_r)(\theta)=\frac{1}{2\pi}\int^\pi_{-\pi}P_r(\theta-t)f(e^{it})dt\]
    で定めると，これは$\Delta$上調和的で，$T=\partial\Delta$上で殆ど至る所極限を持って$f$に一致する．
\end{proposition}

\begin{proposition}
    $\{P_r\}\subset L^1(T)$は線形作用素$L^1(T)\to\C$として，$r\to 1$についてデルタ関数に各点収束する．
\end{proposition}

\subsection{補空間を持たない部分空間}

\begin{notation}
    $X$を位相線型空間，$G$を位相群，作用$G\times X\to X$は連続，$(T_s):G\to\Hom_\TVS(X)$は群準同型であるとする．
\end{notation}

\begin{theorem}[作用素の群]
    $X$をFrechet空間，$Y\subset X$を補空間を持つ部分空間とする（閉とは限らない）．$G$は$\End_\TVS(X)$に作用するコンパクト群で，$\forall_{s\in G}\;T_s(Y)\subset Y$とする．
    このとき，任意の連続な射影$Q:X\epi Y$は任意の$T_s$と可換である．
\end{theorem}

\begin{example}[負の係数が消える関数の空間]
    $\bS:=\partial\Delta\subset\C$を単位円周，
    $X=L^1(\bS)$，$H^1:=\Brace{f\in L^1(\bS)\mid\forall_{n<0}\;\wh{f}(n)=0}$とする．
    群作用$\act:G\to L^1(\bS)$を，$e^{is}\in G$に対して平行移動作用素$(\tau_sf)(\theta)=f(s+\theta)$と定めることで$L^1$に作用する．このとき，$\forall_{s\in\R}\;\tau_s(H^1)=H^1$が成り立つ．
    ここから$H^1$は補空間を持たないことが，Poisson核を用いた帰謬法によって分かる．
\end{example}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    畳み込みは平行移動と微分と可換であることが肝要である．
    実際，微分とはDirac測度の微分との畳み込みと見れる．
\end{tcolorbox}

\begin{notation}
    $u:\R^n\to\R$，$x\in\R^n$について，
    \begin{enumerate}
        \item $(\tau_xu)(y):=u(y-x)$を始点を$x$に取り替える平行移動作用素とする．
        \item $\check{u}(y):=u(-y)$を反転とする．
        \item $(\tau_x\check{u})(y)=\check{u}(y-x)=u(x-y)$に注意．
    \end{enumerate}
\end{notation}
...
\begin{definition}
    $*:L^1(\R^n)\times L^p(\R^n)\to L^p(\R^n)\;(p\in[1,\infty])$を次のように定める
    \[(u*v)(x):=\int_{\R^n}u(y)(\tau_x\check{v})(y)dy=\int_{\R^n}u(y)v(x-y)dy.\]
\end{definition}

\begin{lemma}[Young]\mbox{}\label{lemma-Young-for-convolution}
    \begin{enumerate}
        \item そもそも$u(y)v(x-y)$は殆ど全ての$x$について$y\in\R^n$上可積分である．
        \item $f*g\in L^p(\R^n)$も成り立つ．
        \item 劣乗法性を持つ：$\norm{f*g}_{L^p}\le\norm{f}_{L^1}\norm{g}_{L^p}$．
    \end{enumerate}
\end{lemma}

\begin{proposition}[一般化されたYoungの不等式]
    $f\in L^p(\R^N),g\in L^q(\R^N)\;(1\le p,q\le N)$とし，$\frac{1}{r}=\frac{1}{p}+\frac{1}{q}-1\ge0$とする．このとき，
    $f*g\in L^r(\R^N)$であり，$\norm{f*g}_{L^r}\le\norm{f}_{L^p}\norm{g}_{L^q}$．
\end{proposition}

\begin{proposition}
    $f\in L^1,g\in L^p,h\in L^{p^*}$について，
    \[\int(f*g)h=\int g(\check{f}*h).\]
\end{proposition}

\subsection{台の縮小}

\begin{proposition}[畳み込みは台を縮小させる]
    $f\in L^1,g\in L^p$について，$\supp(f*g)\subset\supp f+\supp g$．
\end{proposition}

\subsection{コンパクト台を持つ連続関数との畳み込み}

\begin{proposition}
    $f\in C_c,g\in L^1_\loc$のとき，$f*g\in C$．
\end{proposition}

\begin{proposition}
    $f\in C_c^k,g\in L^1_\loc\;(k\in\N\cup\{\infty\})$とする．このとき，$f*g\in C^k$かつ$D^k(f*g)=(D^kf)*g$．
\end{proposition}

\subsection{超関数の畳み込み}

\begin{definition}
    $u\in\D',\phi\in\D$について，
    \begin{enumerate}
        \item $(u*\phi)(x):=u(\tau_x\check{\phi})$．特に$u\in L^1_\loc$のとき，自然なペアリングの下で元の定義と一致する．
        \item $u$の平行移動を$(\tau_xu)(\phi):=u(\tau_{-x}\phi)$で定めると，$\tau_xu\in\D'$である．
    \end{enumerate}
\end{definition}

\begin{theorem}
    $u\in\D',\phi,\psi\in\D$について，
    \begin{enumerate}
        \item $\forall_{x\in\R^n}\;\tau_x(u*\phi)=(\tau_xu)*\phi=u*(\tau_x\phi)$．
        \item $u*\phi\in C^\infty$かつ任意の多重指数$\al$について$D^\al(u*\phi)=(D^\al u)*\phi=u*(D^\al\phi)$．
        \item $u*(\phi*\psi)=(u*\phi)*\psi$．
    \end{enumerate}
\end{theorem}

\subsection{畳み込みの特徴付け}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    畳み込みは，
    任意の偏微分作用素と可換な線型作用素として，
    そして平行移動と可換な作用素として特徴付けられる．
\end{tcolorbox}

\begin{theorem}[畳み込み作用素の特徴付け]
    次の2条件は同値：
    \begin{enumerate}
        \item $L:\D\to C^\infty$は連続な作用素で，$\forall_{x\in\R^n}\;\tau_xL=L\tau_x$を満たす．
        \item ある唯一の$u\in\D'$について$\forall_{\phi\in\D}\;L\phi=u*\phi$と表せる．
    \end{enumerate}
\end{theorem}

\begin{proposition}
    $L:\D\to C^\infty$を連続線型作用素で，任意の$D^\al$と可換であるとする．このとき，ある$u\in\D'$が存在して，$L\phi=u*\phi$と表せる．
\end{proposition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    畳み込みの単位元はディラック関数$\delta_0$であるが，これは存在しない．そこで，近似的単位元を考えると，これは元の関数を殆ど変えないわけであるから，うまく滑らかな列を取れば，軟化子として使える．
\end{tcolorbox}

\begin{definition}[approximate identity / mollifier]
    列$\{\rho_n\}\subset C_c^\infty(\R^n)$が\textbf{近似的単位元}または\textbf{軟化子}であるとは，ある$\rho\in C_c^\infty(\R^n)_+,\int\rho>0$に対して
    $\rho_j(x):=j^n\rho(jx)\;(j\in\N)$と構成された列をいう．
\end{definition}
...
\end{lemma}
\begin{Proof}
    \[\rho(x):=\exp\paren{\frac{1}{\abs{x}^2-1}}1_{B^\circ}\]
    とすれば良い．
\end{Proof}

\begin{proposition}
    $(\rho_j)$を軟化子とすると，次を満たす：
    \begin{enumerate}
        \item $\forall_{\phi\in\D}\;\lim_{j\to\infty}\phi *h_j=\phi$．
        \item $\forall_{u\in\D'}\;\lim_{j\to\infty}u*h_j=u$．
    \end{enumerate}
\end{proposition}
\begin{remarks}
    (2)は$\D'$の元は$C^\infty$の列の極限として表せることを主張している．
\end{remarks}

\subsection{軟化作用素と変分法の基本補題}


\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
    title=]
    $L^p$の元を$C^\infty$の列で近似するときはいつでも畳み込みを用いる．
    隆起関数との畳み込みは，任意の尺度$\ep>0$に対して，$u\in L_\loc^1(\R^n)$上の台を$\ep$以上は大きくしない範囲で，ノルム減少的に滑らかにすることが出来る．
\end{tcolorbox}
...
\begin{definition}[mollifier]
    ...
    \begin{enumerate}
        ...
        \item 任意の$\ep>0$に対して，畳み込みを取る作用素を$J_\ep:L^1_\loc(\R^n)\to C^\infty(\R^n);u\mapsto J_\ep u=j_\ep*u$で定める．これを\textbf{Friedrichsの軟化子}という．
    \end{enumerate}
\end{definition}
\begin{lemma}
...
\begin{theorem}[du Bois-Reymond lemma / 変分法の基本補題]
    ...
    すなわち，任意の$u\in L^1_\loc(\Om)$に対して，
    \[\forall_{\varphi\in C_c^\infty(\Om)}\quad\int_\Om u(x)\varphi(x)dx=0.\]
\end{theorem}

\subsection{コンパクト台を持つ超関数の畳み込み}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    超関数のほうがコンパクト台を持つならば，もう一方は$\D$ではなく$C^\infty(\Om)$としても良い．
    同様の発想で，超関数の間にも畳み込みは定義出来る．
\end{tcolorbox}

\begin{theorem}
    $\Lambda\in\D'(\Om)$の台を$S_\Lambda\subset\Om$とする．
    \begin{enumerate}
        \item $\forall_{\phi\in\D(\Om)}\;\supp\phi\cap S_\lambda=\emptyset\Rightarrow\lambda\phi=0$．
        \item $S_\Lambda=\emptyset\Rightarrow\Lambda=0$．
        \item $\psi\in C^\infty(\Om)$がある$S_\Lambda\subset V\osub\R^n$にて$\psi(V)=\{1\}$を満たすならば，$\psi\Lambda=\Lambda$．
        \item $S_\Lambda$がコンパクトならば，$\Lambda$は有限次数を持ち，$\exists_{C\in\R}\;\exists_{N\in\N}\;\forall_{\phi\in\D(\Om)}\;\abs{\Lambda\phi}\le C\norm{\phi}_N$．さらに，$\Lambda$は$C^\infty(\Om)$上への一意な延長を持つ．
    \end{enumerate}
\end{theorem}

\begin{theorem}
    任意の連続線型汎関数$C^\infty(\Om)\to\C$は，あるコンパクト台を持つ分布の延長である．
\end{theorem}

\begin{definition}
    $u\in\D'$はコンパクト台を持つとする．このとき，唯一の連続延長$\o{u}:C^\infty\to\C$が存在するから，これを通じてより広いクラス$\phi\in C^\infty$に対して$(u*\phi)(x):=u(\tau_x\check{\phi})$と定められる．
\end{definition}

\begin{theorem}
    $u\in\D'$はコンパクト台を持ち，$\phi\in C^\infty,\psi\in\D$とする．このとき，
    \begin{enumerate}
        \item $\forall_{x\in\R^n}\;\tau_x(u*\phi)=(\tau_xu)*\phi=u*(\tau_x\phi)$．
        \item $u*\phi\in C^\infty$かつ$D^\al(u*\phi)=(D^\al u)*\phi=u*(D^\al\phi)$．
        \item $u*\psi\in\D$．
        \item $u*(\phi*\psi)=(u*\phi)*\psi=(u*\psi)*\phi$．
    \end{enumerate}
\end{theorem}

\subsection{超関数同士の畳み込み}

\begin{proposition}
    $u_1,u_2\in\D'(\R^n)$を超関数とし，いずれか片方がコンパクト台を持つとする．このとい，$u_1*u_2\in\D'(\R^n)$は，次を満たす唯一の超関数として定まる：
    \[\forall_{f\in C^\infty(\R^n)}\quad(u_1*u_2)*f=u_1*(u_2*f).\]
\end{proposition}

\subsection{緩増加分布の畳み込み}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    スペクトル$C_c^\infty(\Om)\mono\S(\Om)\mono C^\infty(\Om)$がある．
    試験関数の空間を$C_c^\infty(\Om)$とすると，最も大きい双対空間$\D'(\Om)$を得て，ここには種々の使い慣れた関数クラスが埋め込め，$\D'(\Om)$上に微分がTVSの自己同型として定まる．
    $\S(\Om)$にすると，Fourier変換の振る舞いが良い双対空間$\S'(\Om)\subset\D'(\Om)$を得る．さらに$C^\infty(\Om)$まで大きくすると，コンパクト台を持つ超関数のクラス$\E'(\R^n)$が得られる．
    全てはRieszの表現定理の霊性$(C_c(X;\C))^*\simeq_\Ban\RM(X;\C)$の一般化とみなせる．
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    Frechet空間の帰納的極限$\D(\Om)$には，一様構造から定まる自然な位相が入り，これについての双対空間をSchwartz空間という．
\end{tcolorbox}

\begin{remarks}[試験関数の空間の位相]
    $\D(\Om)\;(\Om\osub\R^n)$をコンパクト台を持つ滑らかな関数の空間とする．現代的には$C_c^\infty(\Om)$と書く．
    \begin{enumerate}
        \item $\norm{\phi}_N:=\max\Brace{\abs{D^\al\phi(x)}\in\R_+\mid x\in\Om,\abs{\al}\le N}$は任意の$N\in\N$についてノルムを定める．$N=0$のときは広義一様収束を定める位相である．このノルムについては明らかに完備でない（強位相と呼び分ける）．
        \item $(\D(\Om),\tau)$が局所凸で完備な位相線型空間になる位相$\tau$が存在する．ただし，$(\D(\Om),\tau)$は第一類集合であり，$\tau$は距離化可能ではない．$\D(\Om)$はHaine-Borel性を持つ：有界閉集合はコンパクトである．$\D(\Om)$は回帰的である．
        \item $\{\phi_i\}\subset\D(\Om)$がCauchy列ならば，ある$K\compsub\Om$について$\forall_{N\in\N}\;\lim_{i,j\to\infty}\norm{\phi_i-\phi_j}_N=0$で，収束する．Frechet空間$\D_K$の完備性を利用しているのがよくわかる．
        \item $\phi_i\to0$のとき，やはりある$K\compsub\Om$について$\{\phi_i\}\subset\D_K$で，導関数も$\forall_{\al}\;D^\al\phi_i\to0$に一様収束する．
    \end{enumerate}
\end{remarks}
...
\begin{definition}[distribution]
    $(D(\Om),\tau)$上の連続な線型汎関数を\textbf{$\Om$上の分布}という．
    分布の全体を$\D'(\Om):=\Hom_\TVS(\Om,\R)$で表す．
\end{definition}
\begin{remarks}
    このように定めたために，自明な線形写像$\ev_x:\D(\Om)\to\R$も連続になる．これをデルタ関数という．
    さらに，自然な$L^1$-流のペアリングについて，任意の局所可積分な関数は分布である：$L^1_\loc(\Om)\subset\D'(\Om)$．
    この埋め込みを$f\mapsto\Lambda_f$とする．
    これは$\D'(\Om)$上稠密で，実は$C_c^\infty(X)\subset\D'(X)$も稠密である(変分法の基本補題)．
    また，複素Borel測度$\mu\in M(\Om)$は，これが定める積分が連続になるから，$M(\Om)\subset\D'(\Om)$とみなせる．
\end{remarks}

\begin{theorem}[連続性の特徴付け]
    $\Lambda:D(\Om)\to\R$を線型汎関数とする．次の2条件は同値：
    \begin{enumerate}
        \item 連続である：$\Lambda\in\D'(\Om)$．
        \item 任意のコンパクト集合$K\subset\Om$に対して自然数$N\in\N$と定数$C\in\R$が存在して，$\forall_{\varphi\in D_K}\;\abs{\Lambda\varphi}\le C\norm{\varphi}_N$を満たす．
    \end{enumerate}
\end{theorem}

\begin{proposition}[正な超関数のなす凸錐は測度である]
    $\Lambda\in\D'(\Om)$は正であるとする：$\forall_{\phi\in\D(\Om)}\;\phi\ge0\Rightarrow\Lambda\phi\ge0$．
    このとき，$\Lambda$とは$\Om$上の局所有限な測度である．
\end{proposition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $D:\D(\Om)\to\D(\Om)$は連続なのであった．これを$D:\D'(\Om)\to\D'(\Om)$に延長する．
    すると特に，$C^\infty(\Om)(\subset L^1_\loc(\Om))$上の微分が，デルタ関数などを含む関数クラス$\D'(\Om)$上に延長されたことになる．
\end{tcolorbox}

\begin{definition}
    一般の$\Lambda\in\D'(\Om)$に対して，
    \[(D^\al\Lambda)(\phi):=(-1)^{\abs{\al}}\Lambda(D^\al(\phi))\quad(\phi\in\D(\Om))\]
    と定めると，これは連続である．
\end{definition}

\begin{lemma}[$C^\infty$上での古典的微分との一致]
    埋め込み$\Lambda_-:L^1_\loc(\Om)\to\D'(\Om)$について，$f\in C^{\abs{\al}}(\Om)$ならば，$D^\al\Lambda_f=\Lambda_{D^\al f}$．すなわち，
    \[\forall_{\phi\in\D(\Om)}\quad(-1)^{\abs{\al}}\int_\Om f(x)(D^\al\phi)(x)dx=\int_\Om(D^\al f)(x)\phi(x)dx.\]
    一方で，$f$の連続微分可能性が足りない場合，必ずしも一致しない．
\end{lemma}

\begin{proposition}[商の極限としての特徴付け]
    $u\in\D'(\R)$について，$\D'(\R)$上での弱収束
    \[\frac{u-\tau_xu}{x}\xrightarrow{x\to0}Du\]
    が成り立つ．
\end{proposition}
\begin{remarks}
    本質的には空間を拡張してその上での微分を考えているのみである．
\end{remarks}

\subsection{滑らかな関数による作用}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    積$C^\infty(\Om)\times\D'(\Om)\to\D'(\Om)$が定まる．すると，$C^\infty(\Om)$のFrechet空間としての位相について連続である．
    $\Lambda f$は実数だが，$f\Lambda$は超関数である．
\end{tcolorbox}

\begin{definition}
    $f\in C^\infty(\R^n)$と$u\in\D'(\R^n)$との作用$f\cdot u\in C^\infty(\R^n)$を，超関数$\forall_{g\in\D(\R^n)}\;(f\cdot u)(g):=u(f\cdot g)$によって定める．
\end{definition}

\begin{theorem}
    $\{\Lambda_i\}\subset\D'(\Om)$は$\Lambda:\D(\Om)\to\C$に弱収束するとする．
    このとき，$\Lambda\in\D'(\Om)$で，$D^\al\Lambda_i$も$D^\al\Lambda$に弱収束する．
\end{theorem}

\subsection{コンパクト台を持つ超関数}

\begin{definition}
    開部分集合$\om\subset\Om$について，$\Lambda(\D(\om))=\{0\}$ならば，$\Lambda$は$\om$で消えるという．
    $\Lambda$が消えるような開部分集合の合併$W$の補集合を\textbf{台}という．
    このときたしかに，$\Lambda$は$W$上でも消える
\end{definition}

\begin{definition}
    $C^\infty(\R^n)$には，$K\compsub\R^n$と多重指数$\al$に対応するセミノルム$p_K^\al:C_c^\infty(\R^n)\to\R_+;\phi\mapsto\sup_{x\in K}\abs{D^\al\phi(x)}$が
    誘導する位相が入っているとする．
    Schwartzは$\E:=C^\infty(X)$と表した．
    この双対空間$\E'(X)\subset\D'(X)$で表し，\textbf{コンパクト台を持つ超関数}の空間という．
\end{definition}

\begin{proposition}
    任意の線形作用素$u:C^\infty(\R^n)\to\R$に対して，次の2条件は同値：
    \begin{enumerate}
        \item $u\in\E'(\R^n)$である．
        \item $\exists_{K\compsub\R^n}\;\exists_{k\in\N}\;\exists_{c\in\R_{>0}}\;\forall_{\phi\in C^\infty(\R^n)}\;\paren{\abs{u(\phi)}\le C\sum_{\abs{\al}\le k}\sup_{x\in K}\abs{D^\al K}}$．
    \end{enumerate}
\end{proposition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    対応$C(\Om)\epi\D'(\Om)$は全射である．
    任意の分布は，少なくとも局所的には，ある連続関数の導関数とみなせる．
\end{tcolorbox}

\begin{theorem}
    $\Lambda\in\D'(\Om),K\compsub\Om$をコンパクト集合とする．
    このとき，連続関数$f\in C(\Om)$と多重指数$\al$とが存在して，次を満たす：
    \[\forall_{\phi\in\D_K}\quad\Lambda\phi=(-1)^{\abs{\al}}\int_\Om f(x)(D^\al\phi)(x)dx.\]
\end{theorem}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^1(\R^n)$上にFourier変換を定めた後，像は$C_0(\R^n)$が取れること，$\S_n\subset L^1(\R^n)$上にはBanの同型を定めること，ここから改めて$L^1\cap L^2$そして$L^2$上にHilbの同型として再延長出来ることを観る．
\end{tcolorbox}

\begin{notation}
    $T:=\partial\Delta,U:=\Delta$とする．
    \begin{enumerate}
        \item $C^\infty(T)$は$C^\infty(\R)$のうち，周期$2\pi$を持つ関数のなす部分空間とみなせる．
        ...
        \item $f\in L^1$に対して，指標$e_t(x)=e^{itx}$との畳み込みでの$u=0$の値への対応
...
title=]
    ...
    したがってFourier変換は，指標による積を平行移動に(積を和に．指数法則みたいな)，平行移動を指標による積に変換する，
    群準同型$(L_1,\delta_0,*)\to(L_1,1,\cdot)$を与える．
    すると微分は$it$による積に変換される！
\end{tcolorbox}
...
\begin{definition}[character, characteristic function]\mbox{}
    \begin{enumerate}
        ...
        \item $\R$の連続な指標はすべて指数関数によって表現される．
        \item 指標$t\in\R^n$と$\R^n$上の確率測度との畳み込み$\varphi(u)=(\mu *e_u)(0)=\int_{\R^n} e^{iut}d\mu$を特性関数という．
    \end{enumerate}
\end{definition}
...
\begin{theorem}[Fourier変換の関手性]
    ...
    \begin{enumerate}
        ...
        \item $g\in L^1$に対して$g_n(x):=g(n^{-1}x)$と定めると，$\wh{g_n}(y)=n\wh{g}(ny)$．
    \end{enumerate}
    また，次が成り立つ：
    \begin{enumerate}\setcounter{enumi}{4}
        \item 複素共役：$g(x)=\o{f(-x)}\Rightarrow\wh{g}(t)=\o{\wh{f}(t)}$．
        \item 微分：$g(x)=-ixf(x)\land g\in L^1\Rightarrow\wh{f}$は微分可能で$\wh{f}'(t)=\wh{g}(t)$．
        \item $f\in L^1(\R),\id\cdot f\in L^1(\R)$ならば$\wt{f}$は微分可能で，$(\wt{f})'(y)=-i\wh{\id\cdot f}(y)$．
        \item $f$が微分可能，$f,f'\in L^1(\R)$ならば，$\wh{(f')}(y)=iy\wh{f}(y)$．
    \end{enumerate}
\end{theorem}
\begin{Proof}\mbox{}
    \begin{enumerate}
...
\begin{remarks}
    ...
    形式的には，$\wh{\tau_x}=e_{-x},\wh{e_x}=\tau_x$，$\wh{*}=\cdot$．
...
\subsection{有界作用素としてのFourier変換}

\begin{theorem}
    Fourier変換は
    $\wh{-}:L^1(\R^n)\to\Map(\R^n)$は
    \begin{enumerate}
        \item $B(L^1(\R),C_0(\R))$に属する有界線形作用素であり，ノルム減少的である：$\norm{\wh{f}}_\infty\le\norm{f}_1$．
    \end{enumerate}
\end{theorem}

\subsection{Schwartz空間上のFourier変換}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $\S_n\subset L^1(\R^n)$に話を限るとさらに理想的な振る舞いをする．
    Fourier変換はこの上にTVSの同型を定める．
\end{tcolorbox}

\begin{theorem}
    $f\in\S_n,P$を多項式とする．
    \begin{enumerate}
        \item $\wh{P(D)f}=P\wt{f}$．
        \item $\wh{Pf}=P(-D)\wh{f}$．
    \end{enumerate}
\end{theorem}

\begin{lemma}[Guass分布はFourier変換の不動点になる]
    $\phi_n$を$\R^n$上の分布$N_n(0,I)$の密度関数の$(2\pi)^{n/2}$倍とする：$\phi_n(x)=\exp\paren{-\frac{1}{2}\abs{x}^2}=\exp\paren{-\frac{1}{2}x^\top x}$．
    このとき，$\phi_n\in\S_n$で，$\wh{\phi_n}=\phi_n$が成り立ち，そして
    \[\phi_n(0)=\int_{\R^n}\wh{\phi_n}dm_n.\]
\end{lemma}

\begin{theorem}[inversion theorem]
    $\wh{-}:\S_n\to\S_n$について，
    \begin{enumerate}
        \item $g\in\S_n$ならば，逆$g(x)=\int_{\R^n}\wh{g}e_xdm_n$．
        \item $\wh{-}:\S_n\to\S_n$は，周期4の位相線型空間の同型である．すなわち，全単射な連続線形写像で，逆も連続である．
        \item $f,g\in L^1,n\in\N$について，
        \[\wh{f}(y)g(n^{-1}y)dy=\int f(n^{-1}x)\wh{g}(x)dx.\]
        \item $f,\wh{f}\in L^1(\R^n)$ならば，$\wh{f}(x)=f(-x)\;\ae$，すなわち，
        \[f(x)=\int_{\R^n}\wh{f}e_xdm_n\quad(\ae x)\]
    \end{enumerate}
\end{theorem}
...
\begin{theorem}
    $f,g\in\S_n$とする．
    \begin{enumerate}
        \item $\S_n$は畳み込みについて閉じている：$f*g\in\S_n$．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^2$上への連続延長を考えることが可能で，これはHilbの同型(=ユニタリ変換)，特に\textbf{$L^2$-ノルムについて等長}になる．
    そしてユニタリ作用素になることはParsevalの定理の名前がついている．
\end{tcolorbox}

\begin{lemma}
    $\S_n$は$L^2$上でも$L^1$上でも稠密より，$S_n\subset L^1\cap L^2$上でも稠密である．
\end{lemma}

\begin{theorem}[Plancherel]
    あるユニタリ作用素$F\in B(L^2(\R^n))$が存在して，$F^4=I$を満たし，
    \[\forall_{f\in L^1(\R^n)\cap L^2(\R^n)}\quad Ff(x)=\int f(y)\exp(-ix\cdot y)dy\]
    を満たすものが存在する．
\end{theorem}

\begin{definition}
    $\S_n\to\S_n$の$L^1\cap L^2$上への連続延長をさらに$L^2$上に延長したものを\textbf{Fourier-Plancherel変換}という．
\end{definition}

\begin{proposition}[Parseval's theorem / Rayleigh's energy theorem]
    任意の$f,g\in\S_n$について，そして一般の$f,g\in L^2$について，$(f|g)=(\wh{f}|\wh{g})$．
\end{proposition}
...
\begin{lemma}
    $L^1(\R_+)$は
    \[f\times g(x):=\int^x_0f(y)g(x-y)dy\]
    を積として，可換で非単位的なBanach代数を定める．
    したがって，Gelfand変換の理論が使える．
\end{lemma}

\begin{theorem}\mbox{}
    \begin{enumerate}
        \item Gelfand変換が
        \[\wh{f}(z):=\int^\infty_0f(x)\exp(-xz)dx\quad(f\in L^1(\R_+),z\in\C_+)\]
        によって与えられる．ただし，$\C_+:=\Brace{z\in\C\mid\Re z\ge0}$を右半平面とした．
        \item $\wh{f}$は$\C_+^\circ$上正則であり，無限遠で消える：$\wh{f}\in C_0(\C_+)$
        \item 位相同型$\wh{L^1(\R_+)}\simeq\C_+$が成り立つ．
    \end{enumerate}
    これをLaplace変換とも言う．
\end{theorem}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=さらに詳しく$L^1(\R^n)$の部分空間を調べる]
    $\D(\R^n)\subset\S(\R^n)\subset L^1(\R^n)$と$\S(\R^n)\subset\E(\R^n)$の関係があり，$\F|_{\S_n}$はTVSの同型を定める．
    すると，まずは随伴により$\S'(\R^n)$上にFourier変換が定まる．次に$\E'(\R^n)\subset\S'(\R^n)\subset\D'(\R^n)$に延長する．
\end{tcolorbox}

\subsection{緩増加超関数}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=多項式・絶対連続確率分布のFourier変換]
    連続単射$i:\D(\R^n)\mono\S_n$が定める
    $i^*:\S_n'\mono\D'$によって，部分空間
    $(L^2(\R^n)\subset)\S'_n\subset\D(\R^n)$を考える．
    この上でも随伴によってFourier変換が全く同じ良い性質で定まる．
\end{tcolorbox}

\begin{definition}[tempered distribution, Fourier transform of distribution]\mbox{}
    \begin{enumerate}
        \item 連続単射$i:\D(\R^n)\mono\S_n$(埋め込みではない)を用いて，
        $i^*:\S_n'\mono\D'$と埋め込める．
        すなわち，$\S_n$上への連続な延長を持つ超関数$u:\D(\R^n)\to\R$のクラスを\textbf{緩増加超関数}という．
        \item $u\in\S'_n$のFourier変換は，随伴$\forall_{\phi\in\S_n}\;\wh{u}(\phi)=\brac{u,\wh{\phi}}(=u(\wh{\phi}))$によって定めると，たしかに$\F^*:\S'_n\to\S'_n$で定まる．
    \end{enumerate}
\end{definition}

\begin{proposition}
    $\D(\R^n)$上の点列で，$\S_n$から定まる相対位相では$0$に収束するが，$\D(\R^n)$の位相では収束しないものが存在する．
\end{proposition}

\begin{theorem}
    $\al$を多重指数，$P$を多項式，$g\in\S_n$，$u\in\S_n'$とする．
    \begin{enumerate}
        \item $D^\al u\in\S'_n$．
        \item $Pu\in\S'_n$．
        \item $gu\in\S'_n$．
...
\begin{lemma}[well-definedness]
    $L^1(\R^n)\mono\D'(\R^n)$について，$L^1(\R^n)$としてのFourier変換と随伴として定めたFourier変換とは一致する．
\end{lemma}

\begin{theorem}[Fourier変換はTVSの同型]
    緩増加関数上のFourier変換の随伴$\S'_n\to\S'_n$について，
    \begin{enumerate}
        \item $\S'_n$の周期4の位相線型空間の同型を定める．
        \item $u\in\S'_n$，$P$を多項式とする．このとき，$\wh{P(D)u}=P\wh{u}$かつ$\wh{Pu}=P(-D)\wh{u}$．
    \end{enumerate}
\end{theorem}

\begin{theorem}[群準同型でもある]
    $u_1\in\S'(\R^n),u_2\in\E'(\R^n)$とする．Paley-Wienerの定理より$\wh{u_1}\cdot\wh{u_2}$は常に定まることに注意．このとき，$\wt{u_1*u_2}=\wh{u_1}\cdot\wh{u_2}$．
\end{theorem}

\subsection{多項式のFourier変換}

\begin{example}[多項式のFourier変換]
    $\S'$上での定義によって，初めて多項式のFourier変換が可能になる．
\end{example}

\begin{corollary}[多項式のFourier変換の台による特徴付け]
    超関数$u$について，次の2条件は同値．
    \begin{enumerate}
...
\subsection{Borel測度のFourier変換}

\begin{lemma}
    $\mu\in M(\R^n;\C)$を複素Borel測度とする．Fourier変換の2つの定義
    \begin{enumerate}
        \item 伝統的な定義：$\wh{\mu}(x)=\int_{\R^n}e^{-ix\cdot t}d\mu(t)$．
        \item 緩増加分布としての定義：
    \end{enumerate}
    は一致する．
\end{lemma}
...
\begin{proposition}
    $\mu\in M(\R^n;\C)$を複素Borel測度とする．Fourier変換$\wh{\mu}:\R^n\to\C$は有界で一様連続である．
\end{proposition}

\subsection{コンパクト台を持つ超関数のFourier-Laplace変換}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    コンパクト台を持つ超関数のFourier変換は再び緩増加分布であるが，これは整関数に延長する．この対応を$\F^*:\E'(\R^n)\to C^\infty(\R^n)\to\O(\C^n)$Fourier-Laplace変換という．
\end{tcolorbox}



\begin{proposition}
    $u\in\E'(\R^n)$をコンパクト台を持つ超関数とし，
    $\wh{u}(\zeta):=(u*e_\zeta)(0)$により定まる$\wh{u}:\R^n\to\R$をFourier変換とする．
    このとき，$\wh{u}$は$\C^n$上に$\wh{u}(\zeta):=(u*e_\zeta)(0)$を満たす延長を持ち，これは整関数になる．
\end{proposition}

\begin{remarks}
    $f\in\E'(\R^n)$をコンパクト台を持つ超関数とし，$\wh{f}:\R^n\to\R$を$\C^n$上で扱うことができれば，
    \begin{enumerate}
        \item Fourier変換$\wh{f}(a)\propto\int f(x)e^{ixa}dx$，
        \item Laplace変換$\wt{f}(a)\propto\int f(x)e^{-a}dx$，
    \end{enumerate}
    とは峻別する必要がなくなる．これを\textbf{Fourier-Laplace変換}$\D'(\C^n)\to\D'(\C^n)$という．
\end{remarks}
...
title=コンパクト台を持つ緩増加関数のFourier変換は整関数を定める]
    関数の無限大での減衰挙動と，そのFourier変換の解析性との間に対応がつく．
    ここに注目して，$\F:\S_n\iso\S_n$の適切な制限として同型を得るタイプの結果をPaley-Wienerの定理という．
    古典的な結果は$\D(\R^n)\subset\S(\R^n)$のFourier変換の像を整関数のクラスとして確定させ，超関数の空間$\D'(\R^n)$を詳しく調べるための道具とする．例えば，
    PDEに対する基本解の存在を証明する際の基礎となる，
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    あるクラスの関数のFourier変換は，$\C$のある領域上で正則な延長を持つ．2つ有名なクラスがあり，
    $L^2(\R_+)\subset L^2(\R)$の(少し違うが)Fourier変換$f(z)=(F|e_z)_{L^2(\R_+)}$は上半平面$H^+$への解析接続が存在し，実は$H^2$に属する．
    すなわち，$\F|_{L^2(\R_+)}:L^2(\R_+)\iso_\Hilb H^2(H^+)$．
    また，$L^2((-A,A))\subset L^2(\R)$のFourier変換$f(z)=(F|e_z)_{L^2((-A,A))}$は$\C$上への正則な延長を持ち，さらに増加速度に関して特殊な性質を持つクラスになる(指数型を持つという)．
\end{tcolorbox}

\begin{notation}\mbox{}
    \begin{enumerate}
        \item $H^+:=\Brace{z\in\C\mid\Im(z)>0}$とする．
        \item $H^p(H^+)$を，$H^+$上の正則関数であって，次のノルムを備えるものとする：$\norm{f}_{H^p}:=\sup_{y>0}\paren{\int\abs{f(x+iy)^p}dx}^{1/p}$．
    \end{enumerate}
\end{notation}

\begin{theorem}
    $f\in H(H^+)$を上半平面上の正則関数で
    \[\sup_{0<y<\infty}\frac{1}{2\pi}\int^\infty_{-\infty}\abs{f(x+iy)}^2dx=C<\infty\]
    を満たすとする．このとき，ある$F\in L^2(\R_+)$が存在して，$f(z)=(F|e_z)_{L^2(\R_+)}=\int^\infty_0F(t)e^{itz}dt$と表せ，$C=\norm{F}_{L^2(\R_+)}^2$を満たす．
\end{theorem}

\begin{theorem}
    $A,C>0,f\in H(\C)$を$\abs{f(z)}\le Ce^{A\abs{z}}$を満たす整関数(exponential-type)で，$\norm{f}_{L^2(\R)}<\infty$とする．
    このとき，ある$F\in L^2((-A,A))$が存在して，
    \[\forall_{z\in \C}\quad f(z)=\int^A_{-A}F(t)e^{itz}dt\]
    を満たす．
\end{theorem}

\subsection{Schwartzの一般化}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $\D(\R^n)$は，Fourier変換を通じて，
    特殊な条件を満たすクラスの整関数の空間と同型になる．
    これを超関数$\D'(\R^n)$に延長して，同型な空間を得られることになり，さらに超関数の研究が進む．
\end{tcolorbox}
...
\begin{theorem}[Paley-Wiener]
    試験関数$\phi\in\D(\R^n)$と複素関数$f:\C^n\to\C$について，次の2条件は同値．
    \begin{enumerate}
        \item ある試験関数$\phi\in\D(\R^n)$が存在して$\exists_{r>0}\;\supp\phi\subset rB$を満たし，これについて$f$はFourier変換$f(z)=(\phi*e_{z})(0)$
        \[f(z)=\int_{\R^n}\phi(t)e^{-iz\cdot t}dm_n(t)\quad(z\in\C^n)\]
        で表せる．
        \item $f$は整関数で，$\exists_{\gamma_N<\infty}\;\forall_{N\in\N}\;\forall_{z\in\C^n}\abs{f(z)}\le\gamma_N(1+\abs{z})^{-N}e^{r\abs{\Im z}}$を満たす．
    \end{enumerate}
\end{theorem}

\begin{remarks}
    \[\E(\C^n):=\Brace{F\in\O(\C^n)\;\middle|\;\exists_{B\in\R}\;\forall_{N\in\N}\;\exists_{C_N\in\R}\;\forall_{\xi\in\C^n}\;\abs{F(\xi)}\le C_N(1+\abs{\xi})^{-N}\exp(B\abs{\Im\xi})}\]
    とすると，この空間は$C_c^\infty(\R^n)$とFourier変換を通じて位相線型空間として同型になる．
\end{remarks}

\subsection{Schwartzの緩増加超関数への一般化}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    コンパクト台を持つ$N$次の超関数の空間$\E'_N(\R^n)\subset\E'(\R^n)$は，整関数のある部分空間と超関数のFourier変換について同型になる．
\end{tcolorbox}
...
\begin{remarks}
    \[\E_N(\C^n):=\Brace{F\in\O(\C^n)\;\middle|\;\exists_{B,C\in\R}\;\forall_{\xi\in\C^n}\;\abs{F(\xi)}\le C_N(1+\abs{\xi})^{N}\exp(B\abs{\Im\xi})}\]
    とすると，これはコンパクト台を持つ$N$次の超関数の空間と同型になる．
\end{remarks}
...
\begin{proposition}
    整級数
    $f(z)=\sum^\infty_{n=0}a_nz^n$は$\Delta$上コンパクト一様収束するとする．このとき，次の3条件は同値：
    \begin{enumerate}
        \item $\exists_{p,\lambda\in\R}\;\forall_{n\ge1}\;\abs{a_n}\le\lambda n^p$．
        \item $\exists_{p,\lambda\in\R}\;\forall_{z\in\Delta}\;\abs{f(z)}\le\lambda(1-\abs{z})^{-p}$．
        \item $\forall_{\phi\in C^\infty(T)}\;\lim_{r\to 1}\int^\pi_{-\pi}f(re^{i\theta})\phi(e^{i\theta})d\theta\in\C$が存在する．
    \end{enumerate}
\end{proposition}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    次は$\F:L^1(\R^n)\to C_0(\R^n)$の延長を，局所化の技法を用いて試みる．
    すると，標準基底に関する偏微分$\paren{\pp{}{x_i}}^k$を$L^2$-の意味で持つならば，対応する深さに連続微分可能な「修正」を持つ．
\end{tcolorbox}

\begin{definition}[differentiable in $L^2$]
    $\Om\osub\R^n$とする．
    \begin{enumerate}
        \item 可測関数$f\in\Meas(\Om;\C)$が\textbf{局所$L^2$}であるとは，$\forall_{K\compsub\Om}\;\int_K\abs{f}^2dm_n<\infty$を満たすことをいう．
        \item 超関数$u\in\D'(\Om)$が\textbf{局所$L^2$}であるとは，ある局所$L^2$-関数$g\in\L(\Om;\C)$について$u=(g|-)$と表せることをいう．
        \item 関数$f$が\textbf{$L^2$-微分可能}であるとは，局所$L^2$-な超関数$D^\al f$が存在して，
        \[\int_\Om g\phi dm_n=(-1)^{\abs{\al}}\int_\Om fD^\al\phi dm_n\]
        を満たすことをいう．
    \end{enumerate}
\end{definition}
...
\begin{theorem}[Sobolev]
    自然数
    $n>0,p,r\ge0$は$r>p+n/2$を満たすとする．
    $f\in\Map(\Om;\C)$は局所$L^2$な超関数微分$D^k_if:=\partial_{x_i}^kf$を$i\in[n],k\in[r]$について持つとする．
    このとき，次が成り立つ：
    \begin{enumerate}
        \item ある$p$階連続微分可能な関数$f_0\in C^p(\Om)$が存在して，$f_0=f\;\ae\; x\in\Om$．
        \item $\abs{\al}\le r$を満たす任意の多重指数$\al$について，$D^\al f$は局所$L^2$である．
    \end{enumerate}
\end{theorem}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $L^p(I)\mono\D'(I)$は超関数の意味で微分を許すが，この超関数微分$D:\D'(I)\iso\D'(I)$の像が再び$L^p(I)$に入るとき，弱微分可能であるといい，$u\in W^{1,p}$と表す．
\end{tcolorbox}

\begin{definition}
    $I=(a,b)$を区間とし，$1\le p\le\infty$とする．
    $L^p(I)$の弱微分可能な関数のなす部分空間を
    \[W^{1,p}(I):=\Brace{u\in L^p(I)\;\middle|\;\exists_{g\in L^p(I)}\;\forall_{\varphi\in C^1_c(I)}\;\int_Iu\varphi'=-\int_Ig\varphi}.\]
    で表す．$H^1(I):=W^{1,2}(I)$と表す．
\end{definition}
\begin{remark}
    近似的単位元により，$C^1_c(I)$は$C^\infty_c(I)$上稠密なので，$\varphi\in C_c^1(I)$で十分である．
    また，LionsやMalliavinによると，$I=\R,p=2$のときは，Fourier変換の言葉によりSobolev空間を定義出来る．
\end{remark}

\begin{theorem}[Sobolev空間はBanach]\mbox{}
    \begin{enumerate}
        \item $W^{1,p}\;(1\le p\le\infty)$は，$l^1$-ノルム$\norm{u}_{W^{1,p}}:=\norm{u}_{L^p}+\norm{u'}_{L^p}$についてBanach空間をなす．また，$l^p$-ノルム$(\norm{u}^p_{L^p}+\norm{u'}^p_{L^p})^{1/p}$でも同値な位相を定める．
        \item $H^1$上の内積$(u,v)_{H^1}:=(u,v)_{L^2}+(u',v')_{L^2}$は$W^{1,2}$-ノルムと同値である．
    \end{enumerate}
\end{theorem}

\begin{theorem}
    $W^{1,p}$は，$1<p<\infty$について回帰的であり，$1\le p<\infty$において可分である．
\end{theorem}
\begin{remarks}
    この回帰性が，$C^1$よりも$W^{1,p}$が変分法で使われる所以である．
\end{remarks}
...
\begin{theorem}
    $u\in L^p\;(1<p\le\infty)$について，次の3条件は同値：
    \begin{enumerate}
        \item $u\in W^{1,p}$．
        \item $\exists_{C\in\R}\;\forall_{\varphi\in C^\infty_c(I)}\;\Abs{\int_Iu\varphi'}\le C\norm{\varphi}_{L^{p^*}(I)}$．
        \item ある定数$C\in\R$が存在して，任意の開集合$\om\subset\subset I$と任意の$h\in\R$について，$\abs{h}<\dist(\om,I^\comp)\Rightarrow\norm{\tau_hu-u}_{L^p(\om)}\le C\abs{h}$．
    \end{enumerate}
    (2),(3)において，$C=\norm{u'}_{L^p}$と選べる．
\end{theorem}
