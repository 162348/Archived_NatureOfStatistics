...
\begin{proof}\mbox{}
    \begin{enumerate}
        ...
        \begin{description}
            ...
            $L^2(\Om)$は可分であるから，可算な正規直交基底$(e'_n)$を取れる．
            このとき，$(e_i'\otimes e_j')_{(i,j)\in\N^2}$は$L^2(\Om\times\Om)$の正規直交基底になることが，Fubiniの定理から分かる．
            これを改めて$(e_i)_{i\in\N}$とする．
            これに対して，$K_n:=\sum_{i=1}^n(K|e_i)e_i\in L^2(\Om\times\Om)$とすると，これらを核とした積分作用素は有限ランクな作用素を定め，$K$にノルム収束する．実際，Besselの不等式とParsevalの等式より，
            \begin{align*}
                \norm{K_n-K}=\Norm{\sum_{i=1}^n(K|e_i)e_i-K}&=\Norm{\sum^\infty_{i=n+1}(K|e_i)e_i}\\
                &\le\sum^\infty_{i=n+1}\abs{(K|e_i)}\xrightarrow{n\to\infty}0.
            \end{align*}
            よって，$K_n$を核として定まる積分作用素のノルムも，
            \begin{align*}
                \norm{K_n-K}^2&=\sup_{f\in B}\norm{K_nf-Kf}^2_2\\
                &\le\norm{K_n-K}^2_2\xrightarrow{n\to\infty}0.
            \end{align*}
        \end{description}
    \end{enumerate}
\end{proof}
