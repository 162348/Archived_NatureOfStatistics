...
\begin{quotation}
    ...
    この問題は長い歴史を持つが，その複雑な数理構造を一つ一つ抽出して，公理にまとまったのはやっと20世紀のことである．

    von Misesが標本空間を措定し，数理物理とつながった．相空間$M$も標本空間である(Feller\cite{Feller})．
    位相の構造も見くびれない．
    Kolmogorovが測度論の方法で不可知性・情報をモデリングし，情報理論やエルゴード理論とつながった．
    Measの一般的な性質として，面積も確率も直積構成について良い性質を持たない，これは何を表しているのだろうか．
    集合演算の極限を取ることで，人間の「極限」の概念が自然にモデルに入っており，大標本理論などの技法につながる．
    確率分布は測度であり，函数解析の方法で解析が可能である．
    さらに，確率変数上の作用素とも見れ，ここでやはり数理物理とつながる．

    独立性以前のエントロピーなどの概念は殆ど数学的に確定する：
    \begin{description}
    ...
    \end{description}
    これらの類比はFréchetの研究(1940,\cite{Frechet})において初めて完全に示された．
\end{quotation}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    von Neumannの正則性公理や整礎性などは，事象のためのものとも考えられる．
    すると，測度が要請する可算性も，人間のために作られたように思える．
    特に，非可測集合の存在は選択公理と同値であるというのは，人類の認知論の一つの歴史的メルクマールに到達しているということでもある．
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    標本空間(Merkmalraum)の概念はvon Misesの21から31までの仕事による．
    ここに測度を載せることは自然な発展であり，Kolmogorovがはっきりと公理をまとめた．
    相空間も標本空間である，標本空間はファイバー束と見た方が良いのだろうか？
\end{tcolorbox}
...
\end{axiom}
\begin{history}
    確率論に測度論を導入することは，Borel (1909)の大数の強法則の証明，Wiener (1920-24)のBrown運動の研究で成功を収め，Kolmogorov (1933)が公理にまとめた．
\end{history}

\begin{definition}[random variable, probability distribution / law]
    確率空間を$(\Omega,\calF)$，$(\X,\B)$を可測空間とする．
    \begin{enumerate}
        \item \textbf{$\X$-値確率変数}とは，可測空間の射$X:\Omega\to\X$をいう．
...
\subsection{標本空間の$\sigma$-代数について}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    標本空間の一番大事な構造として$\sigma$-代数があるが，これは情報の増加を表す．
    もっとも直感的な例として，
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=Kolmogorovと伊藤清の特徴：分割から説き起こす]
...
\begin{definition}\mbox{}
    \begin{enumerate}
        \item $\Om$の直和分割であって可測集合のみからなるものを，\textbf{可測同値類による直和分割}という．
        
        \item 可測関数$X:\Om\to S$について，$\F[X]:=X^{-1}(\D(P^X))\lor 2$を，\textbf{$X$で生成される閉$\sigma$-代数}という．
...
\section{集合演算と事象}\label{sec-event}
...
\begin{definition}\mbox{}
    \begin{enumerate}
        \item 確率空間の\textbf{強同型}とは，測度空間として同型で($\sigma$-加法族を押し出す)，かつ，測度を押し出すことをいう．
        \item 確率空間の\textbf{同型}とは，あるfull setが定める部分確率空間同士が強同型であることをいう．これも同値関係を定める．
        \item 正則確率空間$(\R,\mu)$と同型な完備確率空間$(\Om,P)$を\textbf{標準確率空間}という．
    \end{enumerate}
\end{definition}

\begin{lemma}[完備可分距離空間上の確率測度の正則性の特徴づけ]\label{lemma-K-regular}
    完備可分距離空間上の正則確率測度$(S,P)$について，
    \[\forall_{A\in\Dom(P)}\;\forall_{\ep>0}\;\exists_{K\compsub A}\;P(A\setminus K)<\ep.\]
    この性質を$K$-正則という．一般に$K$-正則な確率測度は正則である．
\end{lemma}

\begin{lemma}[Lusin]
    $(S,P)$を完備可分距離空間上の正則確率空間，$T$を可分距離空間，$f:S\to T$を関数とする．次の2条件は同値．
    \begin{enumerate}
        \item $f$は可測．
        \item $\forall_{A\in\Dom(P)}\;\forall_{\ep>0}\;\exists_{K\compsub A}\;P(A\setminus K)<\ep$かつ$f|_K$は連続．
    \end{enumerate}
\end{lemma}

\begin{lemma}\label{lemma-regularity-of-pushout-measure}
    $(S,P)$を完備可分距離空間上の正則確率空間，$T$を可分距離空間，$f:S\to T$を可測関数とする．
    このとき，像測度$f_*P$は$T$上の$K$-正則確率測度である．
\end{lemma}

\begin{theorem}\label{thm-standard-probability-space}
    完備可分距離空間$S$上の正則確率測度$P$は標準である．
\end{theorem}

\begin{example}\mbox{}
    \begin{enumerate}
        \item 可算集合$\Om$上の$\Dom(P)=P(\Om)$を満たす確率測度は標準である．
        \item $\R^\infty$上の正則確率測度は標準である．
    \end{enumerate}
\end{example}
...
\begin{theorem}[正則確率空間の可算直積]\mbox{}
    \begin{enumerate}
        ...
        \item $S_n$上の正則確率測度の列$(P_n)$の直積の完備化$\o{\prod_{n\in\N}P_n}$は$S$上の正則確率測度である．
    \end{enumerate}
\end{theorem}
...
\subsection{弱収束・漠収束の定義と特徴付け}

\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    弱収束と漠収束は同じ位相に関する収束である(一般に$X$がHausdorffのときこれが成り立つ)．
    しかし，前者に対応する$\sigma(M(X),C_b(X))$-位相で$P(X)$は点列コンパクトだが，後者に対応する$\sigma(M(X),C_c(X))$-位相で$P(X)$とは点列コンパクトとは限らず，収束先が確率測度になるとは限らない．
    距離化可能である．
\end{tcolorbox}
...
\begin{theorem}[Portmanteau定理：弱収束の特徴付け (Alexandroff)\footnote{I don' tknow who invented such a nonsensical name for Alexandroﬀ’s theorem.\cite{Bogachev}}]\label{thm-Portmanteau}
    ...
    \begin{enumerate}
        ...
        \item  $\mu_n,\mu$が定める分布関数$F_n,F$と，$\R$上稠密な集合$C$上で，$\forall_{x\in C}\;\lim_{n\to\infty}F_n(x)=F(x)$．
    \end{enumerate}
    また，次とも同値．
    \begin{enumerate}\setcounter{enumi}{6}
        \item 任意の一様連続な有界関数$f$について，$P_nf\to Pf$．なお，有界性の仮定を外しても同値である．
        \item 任意のコンパクト台を持つ連続関数$f\in C_c(\R)$について，$E_{\mu_n}[f]\to E_\mu[f]$\cite{伊藤清}．
...
\begin{proposition}
    $P(X)$の弱収束を定める位相は距離化可能である．
\end{proposition}
\begin{example}
    Prohorovの距離が知られており，これの$X=\R$の場合がLevyの距離である．
\end{example}

\begin{proposition}
    $\Lambda\subset P(X)$について，(1)$\Rightarrow$(2)が成り立ち，$X$が完備なとき逆も成り立つ．
    \begin{enumerate}
        \item 緊密である．
        \item 相対コンパクトである．
    \end{enumerate}
...
title=]
    ...
    $X=\R$のとき，次が成り立つ．
    \begin{enumerate}
        \item $P(X)$上の弱位相＝$\sigma(M(X),C_b(X))$-位相は第２可算で距離化可能である．
    \end{enumerate}
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    Levy距離$d_L$は$P(\R)$上に弱位相を定め，$(P(\R),d_L)$は完備可分距離空間となる．
\end{tcolorbox}
...
\begin{theorem}[Prokhorov：緊密性とは，相対コンパクト性の特徴付けである]\label{thm-Prokhorov}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    確率測度自体が凸結合の一般化のようなものである．そこで，確率測度も凸結合とその極限について閉じているのは当然のことである．
...
\chapter{有限測度空間上の関数空間}

\begin{quotation}
    弱収束の理論は関数解析の理論の系である．\cite{Yoshida}の120pから．
    確率測度の弱収束とは，$\sigma(P(X),C_b(X))$-位相に関する収束をいう．
\end{quotation}
...
\begin{definition}
    ...
    \[\lim_{\al\to\infty}\sup_{\lambda\in\Lambda}\int_{\Brace{\abs{f_\lambda}\ge\al}}\abs{f}d\mu=0\]
    が成り立つことをいう．
\end{definition}

\begin{lemma}
    族$\F:=\{f_\lambda\}\subset L^1(X)$について，
    \begin{enumerate}
        \item $(X,\A,\mu)$が有限のとき，$\F$が一様可積分ならば，各$f_\lambda$は可積分である．
        \item $\F$が一様有界ならば一様可積分である．
        \item $\mu(X)=\infty$のとき，$\F$が例え一様有界であっても可積分でない$f_\lambda$が存在し得る．
    \end{enumerate}
\end{lemma}

\begin{proposition}[可積分性の特徴付け]
...
\begin{proposition}[可積分性の十分条件]
    列$(f_n)$は
    \[\exists_{\ep>0}\quad\sup_{n\in\N}\int\abs{f_n}^{1+\ep}d\mu<\infty\]
    を満たすならば一様可積分である．
    すなわち，部分空間$L^p(X)\subset L^1(X)\;(p>1)$に入っていれば一様可積分である．
\end{proposition}

\subsection{概収束列の一様可積分性の特徴付け}

\begin{theorem}[概収束列の一様可積分性の特徴付け]
    $(X,\A,\mu)$を有限とし，$f_n\to f\;\as$とする．
    (1)$\Rightarrow$(2)が成り立ち，$f,f_n$が非負のとき(2)$\Rightarrow$(1)も成り立つ．
    \begin{enumerate}
        \item $\{f_n\}$が一様可積分である．
        \item $\{f_n\}\subset L^1(X)$かつ$f\in L^1(X)$で，
...
\begin{corollary}[概収束する可積分関数列の一様可積分性の特徴付け]
    $(X,\A,\mu)$を有限とし，$\{f_n\}\subset L^1(X)$は$f\in L^1(X)$に概収束するとする．
    このとき，次の3条件は同値：
    \begin{enumerate}
        \item $f_n$は一様可積分である．
        \item $f_n\to f\in L^1(X)$．
        \item $\norm{f_n}_{L^1(X)}\to\norm{f}_{L^1(X)}$．
    \end{enumerate}
\end{corollary}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    先程は$f_n$の$L^1$-収束で特徴付けた．
    今回は，$f_n$が定める不定積分が，集合関数$\A\to\R$として各点収束するならば一様積分であることを論じる．
\end{tcolorbox}

\begin{notation}
    $(X,\A,\mu)$を有限測度空間とする．
    \begin{enumerate}
        \item 対称差が零という関係による同値類を$\wt{\A}$で表す．
        \item $\wt{\A}$には距離$\rho(\wt{E},\wt{F}):=\mu(E\triangle F)$が定まる．
        \item 等長な位相の埋め込み$\wt{\A}\mono L^1(\X)$が存在する．
    \end{enumerate}
\end{notation}
...
\begin{theorem}[Vitali-Hahn-Saks]
    $(X,\A,\mu)$を有限測度空間，$\{\nu_n\}\subset M(X)$を全変動が有限な符号付測度の列で，次を満たすとする：
    \begin{enumerate}
        \item $\forall_{n\in\N}\;\nu_n\ll\mu$．
        \item $\forall_{A\in\A}\;\lim_{n\to\infty}\nu_n(A)=:\nu(A)\in\R$．
    \end{enumerate}
    このとき，次の2条件が成り立つ：
    \begin{enumerate}
        \item $(\lambda_n)$は一様に絶対連続である：$\mu(E_\lambda)\to0$ならば，一様に$\nu_n(E_\lambda)\to0$．
        \item 極限関数$\nu:\A\to\R$は$\sigma$-加法的である．
    \end{enumerate}
\end{theorem}

\begin{corollary}
    $(X,\A,\mu)$を有限測度空間とし，$\{g_n\}\subset L^1(X)$とする．
    \[\forall_{E\in\A}\;\lim_{n\to\infty}\int_Eg_nd\mu=:\nu(E)\in\R\]
    ならば，次の2条件が成り立つ．
    \begin{enumerate}
        \item $\{g_n\}$は一様可積分である．
        \item 極限関数$\nu:\A\to\R$は$\A$上$\sigma$-加法的である．
    \end{enumerate}
\end{corollary}
...
\subsection{弱点列コンパクト集合の特徴付け}

\begin{theorem}
    $(X,\A,\mu)$を有限測度空間とし，$\F\subset L^1(X)$を部分集合とする．次の2条件は同値：
    \begin{enumerate}
        \item $\F$は弱点列コンパクトである．
        \item $\F$はノルム有界かつ一様可積分である．
        \item 定積分の族$\Brace{\int_-\abs{f}d\mu}_{f\in\F}$は一様に絶対連続である．
    \end{enumerate}
\end{theorem}
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item $\F$は弱相対コンパクトである．
        \item $\abs{\F}:=\Brace{\abs{f}\in L^1(X)\mid f\in\F}$は弱相対コンパクトである．
    \end{enumerate}
    また，一様可積分性も同値．
\end{corollary}
...
\begin{proposition}
    $(\Om,\F,P)$を有限測度空間とする．
    $L(\Om)$は距離化可能である．例えば，次は距離を定める：
    \begin{enumerate}
        \item $d_1(X,Y):=\inf\Brace{a>0\mid P[\abs{X-Y}>a]<a}$．
        \item $d_2(X,Y):=\inf\Brace{P[\abs{X-Y}>a]+a\in\R_+\mid a>0}$．
    \end{enumerate}
    そしてこの距離に関する収束は確率収束である．
\end{proposition}
...
\begin{theorem}[さらに弱めた逆：Skorohodの概収束実現定理]
    必ずしも同一の確率空間上に定義されているとは限らない実確率変数列$X_n$は$X$に分布収束するとする．
    このとき，ある確率空間の上に実確率変数$(\wt{X}_n),\wt{X}$が存在して，$\wt{X}_n\overset{d}{=}X_n,\wt{X}\overset{d}{=}X$かつ$\wt{X}_n\to\wt{X}\;\as$が成り立つ．
\end{theorem}
...
\begin{proposition}[\cite{西尾}]
    任意の確率変数列
    $\{X_n\}\subset\L(\Om)$について，
    集合$\Delta:=\Brace{\om\in\Om\mid\lim_{n\to\infty}X_n(\om)\in\R}$は可測である．
\end{proposition}
\begin{remarks}
    $\Delta$が充満集合であるとき，$X_n$は概収束するという．
    収束先は$L(\Om)$や$L^1(\Om)$上では一意だが，普通は$X(\om):=1_\Delta \lim_{n\to\infty}X_n(\om)$とすると，可測関数である．
\end{remarks}

\begin{proposition}[確率的Cauchy列としての特徴付け]
    確率変数列
    $\{X_n\}\subset\L(\Om)$について，次の2条件は同値：
    \begin{enumerate}
        \item $(X_n)$は概収束する．
        \item $\forall_{\ep>0}\;\lim_{n\to\infty}P[\exists_{N\in\N}\;\forall_{n,m\ge N}\;\abs{X_n-X_m}<\ep]=1$．
    \end{enumerate}
    (2)の条件が成り立つとき，\textbf{確率的基本列}(fundamental in probability)という(Billingsley 76\cite{Billingsley76})．
\end{proposition}

\begin{proposition}
    確率変数列
    $\{X_n\}\subset\L(\Om)$が確率収束するならば，概収束する部分列が存在する．
\end{proposition}
\begin{remarks}
    Egorovの定理との関係は？
    この$>\ep$が潰せないもどかしさは，他に集積点がある場合を意味しているのだろうか？？
\end{remarks}
...
\subsection{独立確率変数列の収束}

\begin{theorem}
    独立確率変数の列$(X_n)$の級数の過程$S_n:=\sum_{i=1}^nX_i$について，確率収束と概収束とは同値になる．
\end{theorem}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    分布が弱収束するからと言って，モーメントも収束するとは限らない．
\end{tcolorbox}

\begin{example}
    任意の実数列$\{\sigma^2_n\}\subset\R$であって$\sigma^2_n\ge1$を満たすものに対して，$(0,\sigma^2_n)$に従う分布の列で，$N(0,1)$に弱収束するものを作ることが可能である．
\end{example}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $p\in[0,\infty]$が小さいほど$\L^p(X)$は大きく，ノルム$\norm{f}_p$は小さくなることを思い出す．
    すなわち，絶対積率$\beta_r:\R_+\to\R$は$r\in\R_+$について$\beta_0=1$から始まる($\sqrt[p]{-}$を無視すれば)単調増加関数である．
    Chebyshevの不等式\ref{cor-Chebyshev-inequality}で，$P[\abs{X}\ge\ep]\le\paren{\frac{\norm{X}_p}{\ep}}^p$
\end{tcolorbox}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    直積を取ることとその極限操作が確率論の数学的基礎づけの中で最も肝要である．
    また，独立性は$P$の集合積に対する関手性だから，数学的には直積と関係が深い．
\end{tcolorbox}

\subsection{Kolmogorovの$\sigma$-加法族}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    圏論的な精神を重んじて，押出の言葉で書く．
\end{tcolorbox}

\begin{theorem}
    分布列$\{\mu_n\},\;\mu_n\in P(\R^n)$がKolmogorovの一貫性条件
    \begin{quote}
        (C) $\forall_{n<m}\;\mu_n=(\pr_n)_*\mu_m$．
    \end{quote}
    を満たすならば，唯一の分布$\mu\in P(\R^\N)$が存在して，$\mu_n=(\pr_n)_*\mu$を満たす．
\end{theorem}
...
title=]
    測度論なる形式は，構成上の困難を持つ．
    多様体と同様（選択公理の下で第２可算ならば可分），可分性などの暗黙の仮定を置くことになる．

    Kolmogorovは確率測度$\mu$にはじめ完備性(零集合の任意の部分集合は可測)のみを仮定していたが，晩年完全性を追加した．
    伊藤清の教科書では可分性も追加した．
    この2つが揃えば，$\R$の部分集合上の$K$-正則確率測度\ref{lemma-K-regular}のみに議論領域を絞ったことになる．
    完全性は像測度に遺伝する．
\end{tcolorbox}

\begin{notation}[\cite{伊藤清}の用語のまとめ]\mbox{}
    \begin{enumerate}
        \item 任意の測度空間$(X,\B,\mu)$に対して存在する完備化$\o{\mu}$を，\textbf{Lebesgue拡大}と呼ぶ．
        \item 確率空間に位相の情報を加えることを考える．定義域がBorel集合族と一致するとき，確率測度をBorelといい，さらに完備でもあるとき，\textbf{正則}という．正則確率測度を\textbf{分布}といい，$P(X)$で表すこととする．
        \item 確率空間の\textbf{強同型}とは，測度空間として同型で($\sigma$-加法族を押し出す)，かつ，測度を押し出すことをいう．
        \item 確率空間の\textbf{同型}とは，あるfull setが定める部分確率空間同士が強同型であることをいう．これも同値関係を定める．
        \item 正則確率空間$(\R,\mu)$と同型な完備確率空間$(\Om,P)$を\textbf{標準確率空間}という．
...
\begin{definition}[perfect (Kolmogorov)]\label{def-perfect-measure}
    完備確率空間$(S,\mu)$の測度$\mu$が\textbf{完全}であるとは，任意の可測関数$f:S\to\R$に沿った押出が正則になることをいう：$f_*\mu\in P(S)$．
    $\mu$が正則なら，完全である\ref{lemma-regularity-of-pushout-measure}．
\end{definition}
...
\begin{theorem}
    確率空間$(S,\mu)$について，次の2条件は同値．
    \begin{enumerate}
        \item $(S,\mu)$は可分完全である．
        \item $\R^1$のある部分集合とその上の$K$-正則な確率測度が存在して，これと強同型である．
    \end{enumerate}
\end{theorem}

\begin{theorem}[完全性の遺伝]
    $\mu$を$S$上の完全確率測度，$f:S\to T$を可測関数とする．
    像測度$\nu:=\mu f^{-1}$は完全である．
\end{theorem}

\begin{theorem}[完全性の公理化の由来]
    $S$は位相空間で，Borel集合$\B(S)$は可算基を持つとする．
    $S$上の完全確率測度$\mu$が$\B(S)\subset\Dom(\mu)$を満たすならば，正則かつ可分である．
\end{theorem}
\begin{remarks}
    これより，$(S,\mu)$が完全ならば，その確率変数$X:S\to\R$による押し出しは$\R$上に分布を定める．
    なお，位相空間$S$が可算開基を持つ（＝第２可算）ならば可分で，かつ，$\B(S)$も可算基を持つ．
    距離空間に関して，第２可算性と可分性は同値．
    すなわち，完備可分距離空間上の完全測度のみを考えれば，必ず$\R$上に押し出せる．
\end{remarks}
...
\begin{quotation}
    ...
    この理論は実解析の級数論に当たり，その一般化が確率論・martingaleである．
    \begin{enumerate}
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    BorelとHausdorffが限定された状況下で論じ，Cantelli (1917)が初めて定式化した．
\end{tcolorbox}
...
\begin{theorem}
    ...
    次の2条件が成り立つとする．
    \begin{enumerate}[({A}1)]
        \item 正規化：$\xi_{n,j}\in\L^2_0(\R^d)$は2乗可積分で，期待値ベクトルは零とする：$E[\xi_{n,j}]=0$．
        \item Lindeberg条件：$\Sigma_n:=\sum^{k_n}_{j=1}\Var[\xi_{n,j}]\in M_d(\R)$は極限$\Sigma:=\lim_{n\to\infty}\Sigma_n$を持ち，さらに次が成り立つ：
...
title=]
    ...
    そう，「連続分布」といったときは$F$が連続であることをいい(すなわち任意の一点集合が零集合であることに同値)，「絶対連続分布」とは別概念である．
    特異な連続分布関数を持つような分布のみ，確率密度関数や確率質量関数に当たるものを持たない．
\end{tcolorbox}

\begin{definition}[singular continuous distribution]\label{def-continuous-point-of-CDF}
    $F$を分布関数とし，
    $f(x):=F(x)-\lim_{y\nearrow x}F(y)\ge0$とする．
    \begin{enumerate}
        \item $f(x)\ne0$であるとき，$x\in\R$を$F$の\textbf{不連続点}といい($\nu\{x\}\ne0$に同値)，$f(x)$を\textbf{飛躍量}という．
        \item $\forall_{\ep>0}\;F(x+\ep)>F(x-\ep)$が成り立つとき，点$x$は$F$の\textbf{増加点}であるという．不連続点ならば増加点である．
        \item 連続な分布関数$F$の増加点全体の集合がLebesgue零集合であるとき，$F$を\textbf{連続特異分布関数}であるという．
        \item 不連続点がfull set：$\nu(D_\nu)=1$であるとき，\textbf{純不連続分布／離散分布}という．不連続点はたかだか可算なので，そこでの値$\nu\{x\}$で定まる．これは離散分布$F$に対応する．逆に，すべての点が$F$の連続点であるとき(すべての一点集合が零集合であることに同値)，これを連続分布という．
    \end{enumerate}
\end{definition}
...
\begin{lemma}[Levy's inversion formula]\label{lemma-反転公式}
    確率測度$\nu$とその特性関数$\varphi$について，
    \begin{enumerate}
...
\begin{theorem}[Bochner]\label{thm-Bochner}
    複素関数$\varphi:\R\to\C$が次の３条件を満たすならば，ある一次元分布$\mu\in\P(\R)$の特性関数である．特に，命題の３条件を満たすならば，特性関数である．
    \begin{enumerate}
        \item $\xi=0$で連続．
        \item $\varphi(0)=1$．
        \item 正定値である．
...
\begin{tcolorbox}[colframe=ForestGreen, colback=ForestGreen!10!white,breakable,colbacktitle=ForestGreen!40!white,coltitle=black,fonttitle=\bfseries\sffamily,
title=]
    $n$次の積率の列$(\al_n:=E[X^n])$に対して，これを持つ確率分布は構成できるか？という問題は，極限定理に関するChebyshev P. L. (1874)の漸近論の研究で初めて議論された(\cite{Encyclopaedia3}, Prokhorov, A.V.担当部分)．
...
\begin{theorem}[一意性の十分条件]
    ...
    \begin{enumerate}
        ...
        \item Carleman条件：$\sum_{n=1}^\infty\frac{1}{a_{2n}^{1/2n}}=\infty$．
    \end{enumerate}
\end{theorem}
...
\begin{thebibliography}{99}
    ...
    \bibitem{丸山}
    丸山徹．『積分と関数解析』．
    確率測度の*弱収束\url{https://core.ac.uk/download/pdf/145720102.pdf}
    \bibitem{清水良一}
    清水良一．(1976)．中心極限定理．
    \bibitem{Yoshida}
